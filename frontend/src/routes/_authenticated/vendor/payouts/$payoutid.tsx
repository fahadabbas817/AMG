...
import { useGetVendorPayout } from '@/features/vendor-dashboard/api/useGetVendorPayout'
...
function PayoutDetailsPage() {
  ...
  const { data: payout, isLoading } = useGetVendorPayout(payoutid)
  ...
  const handleExport = (format: 'pdf' | 'xlsx') => {
    ...
    const filename = `payout-${payout.payoutNumber}.${format}`
    api
      .get(`/vendor/dashboard/payouts/${payout.id}/export?format=${format}`, {
        responseType: 'blob',
      })
      .then((response) => {
        ...
        <CardHeader className='pb-4'>
          ...
          <CardDescription className='flex items-center gap-4 pt-1'>
            <span className='flex items-center gap-1'>
              <Hash className='h-3 w-3' /> Payout #{payout.payoutNumber}
            </span>
            <span className='flex items-center gap-1'>
